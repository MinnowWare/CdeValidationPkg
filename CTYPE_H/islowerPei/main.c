...
@brief C Development Environment (CDE)\n
    Validation of the <em>islower()</em>-function from the standard C library
...
@mainpage
    islower() validation driver for PEI
...
int main(int argc, char** argv) {
    ...
    CDEMOFINE/*MOduleFIleliNE*/((MFNINF(1) "########################## Test islower() in PEI #################\n"));
    CDEMOFINE/*MOduleFIleliNE*/((MFNINF(1) "##################################################################\n"));
//
...
//
    ...
    for (c = 0; c < count; c++) {
        ...
        CDEMOFINE/*MOduleFIleliNE*/((MFNINF(1) "islower(0x%04X) -> %s\n",c,buffer));
