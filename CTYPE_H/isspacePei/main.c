...
@brief C Development Environment (CDE)\n
    Validation of the <em>isspace()</em>-function from the standard C library
...
@mainpage
    isspace() validation driver for PEI
...
int main(int argc, char** argv) {
    ...
    CDEMOFINE/*MOduleFIleliNE*/((MFNINF(1) "########################## Test isspace() in PEI #################\n"));
    CDEMOFINE/*MOduleFIleliNE*/((MFNINF(1) "##################################################################\n"));
//
...
//
    ...
    for (c = 0; c < count; c++) {
        ...
        CDEMOFINE/*MOduleFIleliNE*/((MFNINF(1) "isspace(0x%04X) -> %s\n",c,buffer));
